...
class ProductMarginClassification(models.Model):
    ...
    def _default_price_round(self):
        # Cerquem la precisió amb el nom "Product Price"
        prec = self.env['decimal.precision'].sudo().search(
            [('name', '=', 'Product Price')], limit=1
        )
        # Si trobem la precisió en usem els digits, si no, agafem
        # els decimals de la moneda de la companyia
        digits = prec.digits if prec else self.env.company.currency_id.decimal_places
        # Retornem 10^(-digits)
        return 10 ** (-digits)
