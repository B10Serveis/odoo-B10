...
class InterCompanyRulesConfig(models.TransientModel):
    ...
    po_from_so = fields.Boolean(
        ...
        help=(
            "Generate a Purchase Order when a Sales Order with this company "
            "as customer is confirmed.\n The intercompany user must at least be "
            "Purchase User."
        ),
        readonly=False,
    )
    purchase_auto_validation = fields.Boolean(
        ...
        help=(
            "When a Purchase Order is created by a multi company rule for "
    ...
    intercompany_purchase_user_id = fields.Many2one(
        ...
        help=(
            "User used to create the purchase order arising from a sales "
