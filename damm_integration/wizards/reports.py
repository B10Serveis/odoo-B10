...
def _g_if_gift(gift_val, nongift_val=""):
    return lambda r, sl: (
        gift_val if sl.discount == 100 or _v_price(sl) == 0 else nongift_val
    )
...
_report_formats = {
    ...
    "sales": [
        ...
        (
            "Importe albarán",
            12,
            lambda r, sl: _v_discount(sl) if sl.discount == 100 else _v_price(sl),
            None,
        ),
        ("Importe Dto", 12, lambda r, sl: _v_discount(sl), None),
        (
            # Modificat perque aparegui DTO VALOR si hi ha descompte per unitat, és el cas de Carboniques Franques.
            "Nº descuento",
            20,
            lambda r, sl: (
            ...
            # "Nº descuento", 20, lambda r, sl: "" if sl.discount in [0, 100] else "DTO VALOR", None,
        ),
    ...
    "conditions": [
        ...
        ("cod_detallista", 50, lambda r, ci: ci.env.context.get("_partner_id"), None),
        (
            "TipoCondición",
            25,
            lambda r, ci: "Descuento Valor" if ci.compute_price == "percentage" else "",
            None,
        ),
...
_report_names = {
    "customers": lambda rep: (
        "Det_%s_%s.txt"
        % (rep.env.company.damm_dealer_code, rep.date_start.strftime("%Y%m%d"))
    ),
    "sales": lambda rep: (
        "%s_VENTAS_Factura_%s.txt"
        % (rep.env.company.damm_dealer_code, rep.date_start.strftime("%Y%m"))
    ),
    "conditions": lambda rep: (
        "CondCiales_%s_%s.txt"
        % (rep.env.company.damm_dealer_code, rep.date_start.strftime("%Y%m%d"))
    ),
}
...
class DammReportsWizard(models.TransientModel):
    ...
    damm_config_ok = fields.Boolean(
        default=(
            lambda self: bool(
                self.env.company.damm_dealer_code and self.env.company.damm_partner_id
            )
        )
    )
    ...
    def _search_report_items(self) -> dict[int, object]:
            ...
            for sale_line in sale.invoice_line_ids:
                ...
                if by_damm is None:  # cache whether product by Damm
                    is_product_by_damm[product.id] = by_damm = (
                        damm_partner_id in product.mapped("seller_ids.partner_id.id")
                    )
                if by_damm:
        ...
        plist_ids = dict(
            ...
            for plist_id, custs in itertools.groupby(
                customers.values(), lambda c: c.property_product_pricelist.id
            )
            if plist_id
        )
        ...
        for plist_item in plist_items:
            ...
            if damm_partner_id in product.mapped(
                ...
                for plcust in plist_ids[plist_item.pricelist_id.id]:
                    conditions[plist_item.id, plcust.id] = plist_item.with_context(
                        _partner_id=plcust.id
                    )

        assert self.report_type == "conditions"
        return conditions

    def _format_report_items(self, items: dict[int, object]) -> list[dict[str, str]]:
        ...
        for item in items.values():
            ...
            for title, maxlen, getter, formatter in formats:
                ...
                if fixed_width:
                    ffmt = "%%%ds" % (
                        maxlen if isinstance(value, (int, float)) else -maxlen
                    )
                if formatter:
    ...
    def _assemble_report(self, lines: list[dict[str, str]]) -> tuple[str, bytes]:
        ...
        if self.report_type in _report_formats_fixed:
            ...
            report_data = lterm.join(
                "".join(line[f] for f in field_names) for line in lines
            )
            if report_data:
                report_data += lterm
        else:
