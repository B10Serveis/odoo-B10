...
class ProductProductInherit(models.Model):
    ...
    def get_product_info_pos(self, price, quantity, pos_config_id):
        ...
        price_per_pricelist_id = (
            pricelists._price_get(self, quantity) if pricelists else False
        )
        pricelist_list = [
        ...
        # Stock Locations - Modificat per mostrar ubicacions enlloc de magatzems!
        warehouse_list = [
            {
                "name": loc.name,
                "available_quantity": self.with_context(
                    {"location": loc.id}
                ).qty_available,
                "forecasted_quantity": self.with_context(
                    {"location": loc.id}
                ).virtual_available,
                "uom": self.uom_name,
            }
            for loc in self.env["stock.location"].search(
                [("usage", "=", "internal"), ("location_id", "not like", "/")]
            )
        ]
            ...
            for s in list(group):
                if not (
                    ...
                    supplier_list.append(
                        {"name": s.partner_id.name, "delay": s.delay, "price": s.price}
                    )
                    break
