...
class B10AccountMove(models.Model):
        ...
        for move in self:
            ...
            if (
                ...
                or (
                    ...
                    and partner_bank
                    and partner_bank
                    in payment_mode.variable_journal_ids.mapped("bank_account_id")
                )
            ):
                continue
            raise models.ValidationError(
                _(
                    "A recipient bank must be set which is valid for "
    ...
    def _onchange_payment_mode_id_b10(self):
        ...
        if (
            is_bank_transfer
            and old_partner_bank
            and new_partner_bank != old_partner_bank
        ):
            raise exceptions.ValidationError(
                _(
                    "The current recipient bank cannot be used "
    ...
    def _compute_partner_bank_id(self):
        ...
        for move in self:
            ...
            def get_bank_id():
                if not payment_mode:
                    return False
                if not bank_transfer or payment_mode.payment_method_id != bank_transfer:
                    return fields.first(
                        move.bank_partner_id.bank_ids.filtered(
                            lambda bank: not bank.company_id
