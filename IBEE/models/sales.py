...
class sale_order_line_IBEE(models.Model):
    ...
    def _compute_amount(self):
        ...
        for line in self:
            ...
            price = (line.price_unit * (1 - (line.discount or 0.0) / 100.0)) + Line_IBEE
...
class account_invoice_line_IBEE(models.Model):
    ...
    def _compute_price(self):
        IBEE = self.product_id.litres_IBEE * float(self.product_id.IBEE) * self.quantity
        self.IBEE_invoice = IBEE
        currency = self.invoice_id and self.invoice_id.currency_id or None
        price = (self.price_unit * (1 - (self.discount or 0.0) / 100.0)) + IBEE
        taxes = False
